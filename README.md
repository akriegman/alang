...
An expression in A is connected together by wires that carry values. The wires are drawn with `-`, `|`, and `+`. Two wires that don't look like they connect do not connect, so eg `+|-` is three disconnected wires next to each other.
